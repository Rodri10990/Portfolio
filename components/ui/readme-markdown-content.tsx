...
const headingClass = "mt-8 break-words font-semibold text-[#10b981] first:mt-0";
...
const markdownComponents: Partial<Components> = {
  h1: ({ children }) => <h1 className={`${headingClass} text-xl sm:text-2xl`}>{children}</h1>,
  h2: ({ children }) => <h2 className={`${headingClass} text-lg sm:text-xl`}>{children}</h2>,
  h3: ({ children }) => <h3 className={`${headingClass} text-base sm:text-lg`}>{children}</h3>,
  ...
  p: ({ children }) => <p className={`my-4 break-words text-sm leading-relaxed ${proseBody} sm:text-base`}>{children}</p>,
  ...
  ul: ({ children }) => <ul className={`my-4 list-disc pl-5 sm:pl-6 ${proseBody}`}>{children}</ul>,
  ol: ({ children }) => <ol className={`my-4 list-decimal pl-5 sm:pl-6 ${proseBody}`}>{children}</ol>,
  ...
  blockquote: ({ children }) => (
    <blockquote className="my-4 border-l-2 border-emerald-500/50 pl-3 text-zinc-400 italic sm:pl-4">{children}</blockquote>
  ),
  table: ({ children }) => (
...
export function ReadmeMarkdownContent({ markdown }: ReadmeMarkdownContentProps) {
  return (
    <div className="readme-md min-w-0 font-terminal text-[0.875rem] md:text-[0.9625rem]">
